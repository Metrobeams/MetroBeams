defmodule PlataformaWeb.UserAuth do
  ...
  @remember_me_cookie "_plataforma_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PlataformaWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_sudo_mode(conn, _opts) do
    if Accounts.sudo_mode?(conn.assigns.current_scope.user, -10) do
      ...
      conn
      |> put_flash(:error, "Você precisa confirmar sua identidade para acessar esta página.")
      |> maybe_store_return_to()
      |> redirect(to: ~p"/users/log-in?reauth=true")
      |> halt()
    end
  end
